...
export type FeedLevel = "ok" | "partial" | "limited" | "stale" | "unknown";

export type FeedVerdict = {
  level: FeedLevel;
  /** Short enough for the header pip. */
  headline: string;
  /** One or two sentences naming the observed share and the newest pass. */
  detail: string;
};
...
export function marketFeedVerdict(
  ...
  // Sold evidence has its own age and provenance rules. Mixing the newest
  // sale into a catalogue-coverage verdict made a fresh listings pass look
  // as though it had failed, and repeated the Sold page's more precise
  // archive warning. Keep this verdict about the stream it actually tests.
  const detail = `${coverageClause}. ${passClause}.`;
  ...
  // A recent, narrow batch is not stale: it is fresh evidence with limited
  // catalogue reach. This distinction matters when a targeted scraper lands
  // hundreds of current rows without re-crawling every row still marked live.
  return { level: "limited", headline: "Limited catalog coverage", detail };
}
